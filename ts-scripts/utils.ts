...
import { CCTPTransfer, CCTPTransfer__factory } from "./ethers-contracts"
...
export interface DeployedAddresses {
  CCTPTransfer: Record<number, string>
}

export function getCCTPTransfer(chainId: number) {
  const deployed = loadDeployedAddresses().CCTPTransfer[chainId]
  if (!deployed) {
    throw new Error(`No deployed hello usdc on chain ${chainId}`)
  }
  return CCTPTransfer__factory.connect(deployed, getWallet(chainId))
}
